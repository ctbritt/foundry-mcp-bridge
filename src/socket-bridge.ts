...
export class SocketBridge {
  ...
  private async handleJobCompleted(data: any): Promise<void> {
    try {
      ...
      // CRITICAL: Foundry v14 workaround - always set both img and background.src
      // The backend provides image_path, but sceneData may not have img field
      if (!scene.img || !scene.background?.src) {
        await scene.update({
          img: data.image_path,
          background: { src: data.image_path }
        });
        console.log(`[foundry-mcp-bridge] Updated scene with img: ${data.image_path}`);
      }
